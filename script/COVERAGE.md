...
The five files in `script/modules/` are draft-canonical. They hold the master text cut into the five modules of the teaching line, assembled from `script/master-script-de.md` and `script/master-script-en.md` by exact passage, without rewriting, summarising or translating. Both master scripts are untouched and remain the authoritative source. The module cut is an operator gate. Once the operator confirms it, the modules become canonical and the masters retire.

Each unit of the nineteen-unit fine cut in `knowledge/drafts/module-map.md` was assigned one language version. Where both masters cover a unit, the fuller version was carried and the other one is recorded below under parallel versions not carried. Material that exists only in the CLARIAH-AT workshop script was not copied, because it is profile material; it is recorded as a gap.
...
| M17 Topic Annotation and Statistical Topic Modeling | none | missing | missing | nothing; the unit is absent from the masters and from the workshop script |
| M18 Project Governance and Research Mission Control | module-05, unit heading only | missing | missing | nothing; the unit is absent from the masters and from the workshop script |
...
Three units carry no master text at all. M15 and M17 have no unit heading in any module. M18 carries a unit heading in module 5 with a note that points at the adjacent material, so that the gap stays visible in the reading order.

What is missing from both masters, independent of the profile material:
...
One version per unit was kept. The other version stays in its master and is listed here with what it adds, so that the operator can decide whether a passage should be merged into the kept unit or become a translation input.

| Unit | Version kept | Version not carried | What the discarded version adds |
| --- | --- | --- | --- |
| M1 | DE abstract and "Zu diesem Skriptum" | EN "Abstract", EN "About These Lecture Notes" | the framing of the master around knowledge work rather than one disciplinary field, which no German passage states |
...
The English master holds a block on how large language models work, covering the training objective against the acquired capabilities, the Transformer architecture, pretraining and posttraining, parametric knowledge against retrievable and contextual information, tokenization, and embeddings with contextual representations. The nineteen-unit map has no unit for it, because the map was drafted against the workshop script, which does not contain it. It is carried in module 1 under M5 and needs either its own unit id or an explicit widening of M5.
...
- Footnote identifiers keep the numbering of their master. A footnote referenced from several modules has its definition repeated in each of them; identifier 12 of the English master appears in modules 1, 2 and 4. The German and English identifier spaces do not collide in any module.
- Module 5 carries both bibliographies and both sets of templates. They are apparatus and not parallel translations of one another. The German list carries two works the English list omits entirely and several that the English master cites only in footnotes, while the English appendix templates carry fields the German ones do not.

## Open questions for the fine-cut confirmation

1. The chapter references in `knowledge/drafts/module-map.md` do not resolve against `script/master-script-de.md`. The map was drafted against a ten-chapter Skriptum in the vault that runs from research data through the four engineering layers, the facsimile pipeline, the Hersch tutorial, topic modeling and project governance to the apparatus. The German master in this repo runs from an introduction through LLM foundations, prompt, context, knowledge and agentic engineering to promptotyping, a summary, a bibliography and templates. Seven units depend on which document is the German source, M2, M11, M13, M14, M15, M17 and M18. Either the vault Skriptum enters the repo as a second German source, or the source column of the map is rewritten against the German master here.
...
4. Taking one language version per unit discards a substantial part of the English master, listed above. Decide per case whether a discarded passage merges into the kept unit or whether the other language version becomes the translation input for the second language.
5. The English block on how large language models work needs a unit id, or M5 needs to be widened explicitly to cover it.
6. Unit M16 is classified as workshop-specific for the CLARIAH profile in the map, yet its material sits in the English master and is therefore carried into module 2. Either the case study leaves the master and becomes profile material, or the classification changes.
7. The German master runs a digital edition as its example throughout, while the English master uses research data workflows and knowledge work in general. The two framings now stand side by side in modules 1 to 4. Decide which framing the master carries and whether the other becomes a profile substitution.
8. The English master names a specific model in the transcription example and in the heading of that section. A heading that carries a version number ages with the model. Decide whether the pin belongs to the documented run or to the module.
...
10. The escaped Markdown and the empty anchors in the German master are an export artefact. Decide whether a cleanup pass runs on the modules after confirmation or on the master before the cut is confirmed.
