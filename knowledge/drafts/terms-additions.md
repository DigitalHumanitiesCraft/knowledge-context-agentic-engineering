...
- **Source.** CLARIAH, "Applied Generative AI for Research Data Workflows"; Notes EN, same section.
...
- **Source.** CLARIAH, "Applied Generative AI for Research Data Workflows" and "The Current LLM Capability Landscape"; Notes EN, abstract and "Which Frontier Models and AI Technologies Do You Use?".
...
- **Source.** CLARIAH, "Frontier LLMs Asymmetrically Amplify Computational Research"; Notes EN, same section; Deck, thesis slide.
...
- **Relation.** The overarching thesis under which the whole capability block of module 1 is organised. It has no row in the matrix.
...
- **Source.** CLARIAH, "Frontier LLMs Asymmetrically Amplify Computational Research" with footnote 26; Notes EN, same section.
...
- **Source.** CLARIAH, "LLMs as Jagged Alien 'Intelligences'" and the closing paragraphs of the amplification section; Notes EN, same section; Deck.
...
- **Source.** Notes EN, "How Large Language Models Work"; Skriptum DE, §2.1.
...
- **Source.** Notes EN, "How Large Language Models Work"; Skriptum DE, §2.1 with footnote 1 marking the description as a functional simplification.
...
- **Source.** Notes EN, "Transformer Architecture"; Deck, context-window slide.
...
- **Source.** Notes EN, "Tokenization"; Skriptum DE, §4.3 in the path from file to context window.
...
- **Source.** Notes EN, "Embeddings and Contextual Representations"; Skriptum DE, §3.6.
- **Literature.** Vaswani et al., arXiv:1706.03762, 2017.
...
- **Source.** Notes EN, "Pretraining and Posttraining" and "Training Builds and Shapes the Latent Program Space"; Skriptum DE, §2.2; CLARIAH, "Training Shapes the Latent Program Space".
...
- **Source.** Notes EN, "Pretraining and Posttraining"; Skriptum DE, §2.2.
...
- **Source.** Notes EN, "Training Builds and Shapes the Latent Program Space".
...
- **Source.** Notes EN, "The Shape of a Wikipedia Article About Zebras" with its three information layers of model parameters, external resources and current context; Skriptum DE, §2.2.
...
- **Source.** Notes EN, "Context Engineering Shapes the Model's Working Context"; the training and inference distinction in "Pretraining and Posttraining".
- **Literature.** Brown et al., arXiv:2005.14165, 2020.
...
- **Source.** CLARIAH, "LLMs as Jagged Alien 'Intelligences'" and the countermeasure in "Practical Prompting Principles"; Notes EN, same two places; Deck.
...
- **Source.** CLARIAH, "Prompt Engineering as Search in a Latent Program Space"; Notes EN, "Prompt Engineering: Finding Coordinates in a Latent Program Space"; Skriptum DE, §3.6.
...
- **Source.** CLARIAH and Notes EN, the five-item terminology list of the latent-program-space sections.
...
- **Source.** CLARIAH, "Reasoning and Test-Time Compute as Internal Search"; Notes EN, the terminology list and the prompting principle "Use Reasoning Selectively".
...
- **Source.** CLARIAH, "Excursus: Mechanistic Interpretability"; Notes EN, "Excursus: Mechanistic Interpretability and Activation Paths"; Skriptum DE, §3.6 with two figures.
...
- **Source.** CLARIAH, "The Assistant Is a Character Generated by the Model"; Notes EN, same section; Skriptum DE, §2.2 with figure 3.
...
- **Source.** Notes EN, "How Anthropic Shapes Claude's Assistant Character"; Skriptum DE, §2.2 with footnote 2 marking the ontological reading as contested.
...
- **Source.** CLARIAH, "Multimodality and Vision-Language Models"; Notes EN, "Multimodality & Vision Language Models".
...
- **Source.** CLARIAH, "Example Result and Critical Review" and "Multimodality and Vision-Language Models"; Notes EN, "Hands on 1 (Example Result)".
- **Literature.** No canonical source. This is the teaching line's own name for the phenomenon and should be marked as such.
...
- **Source.** CLARIAH, "Example Result and Critical Review", closing question; Notes EN, "Multimodality & Vision Language Models".
...
- **Source.** Skriptum DE, §3.2 with the TEI example and §3.8; Notes EN, "Transcribe a Facsimile with Gemini 3.7 Flash".
- **Literature.** No external source. In-house formulation of the teaching line, to be marked as such.
...
- **Source.** CLARIAH, "Prompt Engineering" and the two transcription prompts; Notes EN, "Prompting Strategies", item Structure the Prompt.
...
- **Relation.** Divergence inside the material. The English texts teach four parts, the German Skriptum seven. A glossary entry has to state which list is canonical or that both are instance-dependent.
...
- **Source.** Skriptum DE, §3.3; CLARIAH, "Prompting Is Weird"; Notes EN, "Prompting Is Weird, and Keeps Changing".
...
- **Source.** Skriptum DE, §3.3 with the workshop-participant persona; Notes EN, "Persona Engineering"; Deck.
...
- **Source.** Skriptum DE, §3.4; Deck, hands-on step 3; Notes EN, "Generate, Compare and Reconcile" as the related principle.
...
- **Source.** Skriptum DE, §3.4; CLARIAH, "From Generated Text to Structured Data"; Notes EN, ".txt → JSON → CSV".
- **Literature.** No canonical source. The four levels are the teaching line's own distinction.
...
- **Source.** Skriptum DE, §3.5 with figures 5 and 6 and the seven-step evaluation procedure; CLARIAH, "Prompting Is Weird"; Notes EN, same section.
...
- **Source.** Skriptum DE, §4.2 with the enumeration of what occupies it; CLARIAH, "Context Engineering Shapes the Working Context"; Notes EN, "Context Window, Context Rot and Distillation"; Deck, the two 8K examples.
- **Literature.** Vaswani et al., arXiv:1706.03762, 2017.
...
- **Source.** Skriptum DE, §4.5 with the worked example for one edition page and §4.6 with the four-way loading matrix; Notes EN, "Project Knowledge Base and Working Context"; Deck.
- **Literature.** Pollin, *Promptotyping. Translating Research Data into Research Artefacts through Context Engineering and Agentic Engineering*, review draft 0.9, 2026, which the Full Lecture Notes cites as the source of this formulation.
...
- **Source.** Skriptum DE, §5.3 with the concrete `knowledge/` folder layout; Notes EN, "Project Knowledge Base and Working Context"; Deck.
...
- **Source.** Skriptum DE, §4.2; CLARIAH, "Context Engineering Shapes the Working Context"; Notes EN, "Context Rot"; Deck, with the caveat that the reported evidence is task-specific.
...
- **Source.** Skriptum DE, §4.2 closing paragraph; Notes EN, "Why AI Agents Need Context"; Deck, "Warum AI Agents Context brauchen".
- **Literature.** No canonical source. Formulation of the teaching line, derived from the Context Rot finding.
...
- **Source.** Skriptum DE, §4.4; Notes EN, "Selecting, Structuring and Distilling Knowledge"; Deck.
- **Literature.** Pollin, *Promptotyping*, review draft 0.9, 2026.
...
- **Source.** Skriptum DE, §4.4 with its three operations of selection, structuring and condensation, and §7.4 as a Promptotyping phase; Notes EN, "Selecting, Structuring and Distilling Knowledge"; Deck.
- **Literature.** Pollin, *Promptotyping*, review draft 0.9, 2026.
...
- **Literature.** No canonical source. Term of the teaching line.
...
- **Source.** Skriptum DE, §5.2 with the `<supplied>` example of an unwritten but practised rule; Notes EN, "Scholar Centred Design and Requirements Engineering"; Deck.
...
- **Source.** Notes EN, "'I Know Things'" and "Why Knowledge Must Be Made Explicit"; Skriptum DE, §5.1; Deck.
...
- **Source.** Notes EN, "Knowledge Modelling, Personal Information Management and Project Management"; Deck, the three-tradition slide with its process steps.
...
- **Source.** Notes EN, "Knowledge Modelling, Personal Information Management and Project Management"; Deck, the three-tradition slide.
...
- **Source.** Skriptum DE, §5.6 with the four-way distinction of knowledge document, instruction file, skill and prompt; Notes EN, "AGENTS.md and CLAUDE.md", "Global CLAUDE.md", "Project Specific CLAUDE.md"; Deck.
...
- **Source.** Skriptum DE, §5.6 with edition-specific examples; Notes EN, "Agent Skill"; Deck.
...
- **Source.** Skriptum DE, §5.5; CLARIAH, "Markdown and Knowledge Documents"; Notes EN, "Markdown Makes Document Structure Explicit for LLMs"; Deck.
...
- **Source.** Skriptum DE, §6.4; Notes EN, "Tool Use"; CLARIAH, "LLMs as Jagged Alien 'Intelligences'" for the argument that tool use changes the effective system; Deck.
...
- **Source.** CLARIAH, "Agentic Engineering and AI Harnesses" with the explicit formula; Notes EN, "AI Harness Architecture"; Deck.
...
- **Source.** Notes EN, "Agentic Engineering" with the shift from response to trajectory; Skriptum DE, §6.1 with the propagating-error example; Deck.
...
- **Source.** Skriptum DE, §6.2 with the ten-step edition example; Notes EN, "Agentic Engineering"; Deck, hands-on step 1.
...
- **Source.** Skriptum DE, §6.4 with the edition-specific permission table; CLARIAH, "Prepare the Working Environment before the First Prompt"; Notes EN, same section.
...
- **Source.** Skriptum DE, §6.6 with the reversibility requirement for intermediate states; Notes EN, "Why Multi Step Work Must Be Organised".
...
- **Source.** Skriptum DE, §6.5 with four edition-specific subagent roles; Notes EN, "Subagents" and "Subagents and Epistemic Infrastructure"; Deck.
...
- **Source.** Notes EN, "Model Routing" with the sequence from research through specification, implementation and review to revision; Deck.
...
- **Source.** Skriptum DE, §6.5 with footnote 5 on implementation-dependent permission architecture; Notes EN, "Model Context Protocol (MCP)"; Deck, with the M-times-N argument.
...
- **Source.** Notes EN, "A2A (Agent to Agent)"; Skriptum DE, §6.5 with the four methodological questions on responsibility, handover, conflict and decision authority; Deck.
...
- **Source.** CLARIAH, "AI-Supported Research Data Workflows & Epistemic Infrastructures" and "Agentic Engineering and AI Harnesses"; Notes EN, "Critical Expert and Epistemic Infrastructure"; Deck.
...
- **Relation.** The concept that separates this teaching line from a tooling account. It carries the load in the conclusion of both English texts and has no matrix row.
...
- **Source.** Skriptum DE, ch. 7; CLARIAH, "Promptotyping" with the five-stage flow diagram; Notes EN, "Promptotyping as an Iterative Knowledge Loop".
...
- **Source.** Skriptum DE, §7.5; Notes EN, "Scholar Centred Design and Requirements Engineering"; Deck.
...
- **Working definition.** A requirements format stating who acts, which capability is required and why it matters, used in the material as the bridge between structured research data and an implementable artefact because it is readable by people and usable as agent context.
- **Source.** Notes EN, "As a … I Want to" with the liturgy and social-history examples; Deck, the Scholar-Centred Design slide with worked stories.
...
- **Source.** Notes EN, "Spec Driven Development"; Deck, with the note that the specification is contextualised in the `knowledge` folder.
...
- **Source.** Skriptum DE, §7.9 and §7.11; CLARIAH, "Promptotyping" as curated feedback; Notes EN, "Promptotyping as an Iterative Knowledge Loop" as curated write back.
- **Literature.** Pollin, *Promptotyping*, review draft 0.9, 2026.
...
- **Source.** Notes EN, "Evaluation, Verification, Validation and Acceptance"; CLARIAH, "Evaluation, Validation and Scholarly Verification".
- **Literature.** The Full Lecture Notes marks the broad usage in its own footnote rather than citing a source. Proposed anchor for the model-based case, Zheng et al., arXiv:2306.05685, 2023, already used by the matrix.
...
- **Source.** Skriptum DE, §7.7 with the edition-specific check list and the Verification Report template in the appendix; Notes EN, "Evaluation, Verification, Validation and Acceptance".
- **Literature.** No external anchor in the scripts. The Full Lecture Notes states that the distinction adapts established verification and validation terminology from software and systems engineering.
...
- **Source.** Skriptum DE, §7.7 with its four guiding questions; Notes EN, same section; CLARIAH, "Evaluation, Validation and Scholarly Verification" under the name scholarly verification.
- **Literature.** Pollin, *Promptotyping*, review draft 0.9, 2026.
...
- **Source.** Skriptum DE, §7.7 and the Acceptance Statement template with its sections for identified state, accepted-for purpose, technically verified, disciplinarily checked, not demonstrated, open questions and responsible decision; Notes EN, "Evaluation, Verification, Validation and Acceptance".
- **Literature.** Pollin, *Promptotyping*, review draft 0.9, 2026.
...
- **Source.** Skriptum DE, §7.8; Notes EN, "Critical Expert and Epistemic Infrastructure", which adds that the role designs the epistemic conditions rather than manually checking every output.
...
- **Source.** CLARIAH, "Model-Assisted Review inside an AI Harness"; Skriptum DE, §7.7 with figure 12; Notes EN, "Subagents and Epistemic Infrastructure"; Deck, the three-subagent TEI verification slide.
...
- **Source.** Notes EN, "Hands on 1 (Example Result)"; CLARIAH, "Example Result and Critical Review".
- **Literature.** No canonical source. Term of the teaching line.
...
- **Source.** CLARIAH, "From Generated Text to Structured Data"; Notes EN, ".txt → JSON → CSV" with the pattern from probabilistic interpretation through structured representation to deterministic checking.
- **Literature.** No canonical source. Formulation of the teaching line.
...
- **Source.** CLARIAH, "The Current LLM Capability Landscape" and the cyber-capability passage on diffusion risk; Notes EN, "Which Frontier Models and AI Technologies Do You Use?".
...
- **Source.** CLARIAH, "The Current LLM Capability Landscape" with footnote 23; Notes EN, "Which Frontier Models and AI Technologies Do You Use?".
...
- **Source.** CLARIAH, "Humanities Research Data Are Constructed through Scholarly Workflows"; Notes EN, same section.
...
- **Literature.** No canonical source. Definition of the teaching line, positioned against a pipeline reading.
...
2. **The checking vocabulary is cut three ways inside the material.** The CLARIAH notes name evaluation, validation and scholarly verification, where validation means conformance to a formal or operational requirement. The English Full Lecture Notes names evaluation, technical verification, scholarly or domain validation and acceptance, where validation means domain adequacy and verification means formal conformance. The German Skriptum names technische Verifikation, fachliche und wissenschaftliche Validierung and Acceptance without a separate evaluation level. Verification and validation therefore swap meanings between the CLARIAH notes and the two lecture-notes texts. This is the sharpest terminological conflict found in this pass and it sits on the matrix row that already carries drift note D3.
...
5. **Knowledge Document has two definitions in the material.** The CLARIAH notes say atomic, structured and verifiable unit of externalised project knowledge. The Full Lecture Notes and the Skriptum say bounded, structured and revisable representation distilled from a larger body. Atomic against bounded and verifiable against revisable are different claims, and drift note D4 has to choose.
...
10. **Distillation.** The material term collides with the established machine-learning sense in which a smaller model is trained on a larger one. The audience of the teaching line meets both senses, so the glossary entry needs an explicit disambiguation.
