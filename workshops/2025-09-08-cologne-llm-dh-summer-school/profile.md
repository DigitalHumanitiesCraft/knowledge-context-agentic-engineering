...
Every export was checked for embedded URLs carrying an `ouid=` or `authuser=` query parameter, which would expose the numeric identifier of the owning Google account. None was found in any of the six files, so no query parameter was stripped and the exports are byte-identical to the vault originals. Personal names inside the slide text are research data and stay untouched.

Two observations that the check surfaced and that are left to the operator. Seven LinkedIn URLs across four exports carry an `rcm=` token, which is bound to a LinkedIn member account; the parameter falls outside the cleaning mandate for this intake and was kept. Ten Google URLs are published with these files, meaning six documents (the five scripts and lecture manuscripts plus one deep-research output shown as a demonstration), two earlier decks referenced as prior work, one Drive folder offered to participants for sharing their own material and one file URL of a historical document image used in an exercise. The access state of all ten is unverified, and two further URLs point at shared sessions in hosted assistant products.
